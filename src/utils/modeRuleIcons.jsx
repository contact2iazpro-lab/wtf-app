/**
 * modeRuleIcons — source partagée pour les icônes des règles de mode.
 * Utilisé à la fois par :
 *   - ModeLaunchScreen (page de règles avant partie)
 *   - HowToPlayModal (livret)
 *
 * Garantit que les 2 vues rendent EXACTEMENT les mêmes icônes pour un
 * même `seg.icon` → plus de risque de désync visuelle.
 *
 * Les labels/textes des règles viennent toujours de gameConfig.MODE_CONFIGS,
 * eux aussi consommés par les 2 vues.
 */
...
export const QUICKIE_VIOLET = '#C2185B'
export const QUICKIE_GOLD = '#E91E90'
export const VOF_GREEN = '#6BCB77'
export const VOF_RED = '#E84535'
export const MULTI_VIOLET = '#6B2D8E'
export const MULTI_GOLD = '#FFD700'
export const QUEST_ORANGE = '#FF6B1A'
export const QUEST_DARK = '#D94A10'
export const BLITZ_RED = '#FF4444'
export const DROP_PINK = '#E91E63'
...
export function NoHintIcon({ size = 64, color = '#ffffff' }) {
  return (
...
export function FreeIcon({ size = 64, color = '#ffffff' }) {
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
      <circle cx="50" cy="50" r="38" stroke={color} strokeWidth="6" fill="none" />
...
export function TrophyIcon({ size = 64, color = '#ffffff' }) {
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export function PenaltyIcon({ size = 64, color = '#ffffff', accent = null, text = '−5s' }) {
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
      <circle cx="50" cy="50" r="38" stroke={color} strokeWidth="6" fill="none" />
...
export function TargetIcon({ size = 64, color = '#ffffff', questionMark = false, questionColor = null }) {
  // questionMark : remplace le cercle central par un "?" (ex : Quest boss, Drop set)
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
      <circle cx="50" cy="50" r="38" stroke={color} strokeWidth="6" fill="none" />
...
export function StepsIcon({ size = 64, color = '#ffffff' }) {
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export function SurvivalIcon({ size = 64, color = '#ffffff' }) {
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
...
// ── COMPONENT_ICONS map + renderIcon ───────────────────────────────────────
...
/**
 * Rend une icône à partir d'une clé (`icon:*` ou `picto:*`) ou d'un emoji.
 * @param value - Clé du COMPONENT_ICONS ou emoji Unicode
 * @param size - Taille en px (default 16)
 * @param color - Couleur principale optionnelle
 * @param modeId - Mode courant (pour variants spécifiques)
 */
export function renderIcon(value, size = 16, color, modeId) {
  const compFn = COMPONENT_ICONS[value]
  if (compFn) return compFn(size, color, modeId)
  if (value === '🔋') return <EnergyIcon size={size} color={color} />
