...
import { TUTO_FACT_IDS, TUTO_QUICKIE_CONFIG } from '../constants/gameConfig'
...
export default function TutoTunnel({ onComplete, onSkip }) {
  ...
  useEffect(() => {
    ...
    const config = phase === 'phase1' ? TUTO_QUICKIE_CONFIG : TUTO_QUICKIE_CONFIG
        ...
        <div style={{
          ...
          <button
            onClick={() => {
              ...
              setUsedIds([first.id])
              const factWithOptions = { ...first, ...getAnswerOptions(first, TUTO_QUICKIE_CONFIG) }
  ...
  if (phase === 'phase0') {
    ...
    if (showRevelation && currentFact) {
      ...
      const coinsEarned = isCorrect ? TUTO_QUICKIE_CONFIG.coinsPerCorrect : 0
      return (
        <RevelationTemplate
          catColor={catColor}
    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          Joue en mode Quickie !
        </div>

        {/* Modes grid (all disabled except Quickie) */}
        <div style={{
          display: 'flex', flexDirection: 'column', gap: S_val(16),
          alignItems: 'center', flex: 1, justifyContent: 'center',
        }}>
          {/* Quickie button - highlighted */}
          <button
            onClick={() => {
              audio.play('click')
              ...
              const factsWithOptions = flashFacts.map(f => ({
                ...f,
                ...getAnswerOptions(f, TUTO_QUICKIE_CONFIG),
              }))
          ...
          {['Quest 🎯', 'Quickie 🧭', 'Blitz ⚡'].map((label, i) => (
            <div
              key={i}
              style={{
                padding: S_val(24),
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // PHASE: PHASE1 (5 Quickie questions with real QuestionScreen)
  // ═══════════════════════════════════════════════════════════════════════════
  if (phase === 'phase1') {
    const currentFact = sessionFacts[currentIndex]
